...
2. Follow [setup](guides/setup.md): connect InVideo, install the included skills, then add Remotion and Blender when your edit needs them.
...
| InVideo MCP | Connect the agent to InVideo's available video tools | [Official setup](https://invideo.io/ai/mcp/) · [Connection guide](guides/invideo-mcp.md) |
...
The Blender skill is original guidance from this project, **not an official Blender skill or a Blender MCP server**. Remotion's official skills are linked, not redistributed. **The public InVideo MCP checked for this release offers script-to-video only; it does not expose Seedance selection or reference uploads.** The [connection guide](guides/invideo-mcp.md) includes working setup steps and explains what remains necessary for the reference-driven generation workflow.
...
These resources describe an editing method. The download does not contain the tutorial's private portraits, voice recordings, provider receipts, copyrighted reference clips or finished ads. Use your own assets. The historical tutorial generations used a different provider; this kit's intended generation connection is InVideo. Follow the live tool schema rather than copying a provider-specific request from the video.
